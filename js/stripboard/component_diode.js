...
SC.Diode = function (aName, aX1, aY1, aX2, aY2) {
    this.type = 'diode';
    this.name = aName;
    this.x1 = aX1 || 0;
    this.y1 = aY1 || 0;
...
SC.factory.diode = SC.Diode;
SC.namingPrefix.diode = 'D';

SC.Diode.prototype.toObject = function () {
...
SC.Diode.prototype.fromObject = function (aObject) {
...
SC.Diode.prototype.toNetlist = function () {
...
SC.Diode.prototype.nets = function () {
...
SC.Diode.prototype.flip = function () {
    // Flip pins (polarity)
    [this.x1, this.y1, this.x2, this.y2] = [this.x2, this.y2, this.x1, this.y1];
};

SC.Diode.prototype.hasPin = function (aX, aY) {
    // Return true if given coords are at any of the pins
    return (this.x1 === aX && this.y1 === aY) || (this.x2 === aX && this.y2 === aY);
};

SC.Diode.prototype.distanceTo = function (aX, aY) {
    // Return distance to given point on grid
    return CA.distancePointLineSegment(aX, aY, this.x1, this.y1, this.x2, this.y2);
};

SC.Diode.prototype.render = function (aContext) {
    // Render component
    SC.renderDefault(aContext, this.x1, this.y1, this.x2, this.y2, SC.image.diode, this.name, this.value, 49, 24);
};

SC.Diode.prototype.propertiesDialog = function (aCallback) {
